...
func (e *encoder) writeLiteral(lit byte) error {
	var err error
	...
	err = e.state.litCodec.Encode(e.re, lit, state, match, litState)
	if err != nil {
		return err
	}
...
func (e *encoder) writeMatch(distance int64, length int) error {
	var err error
	if !(minDistance <= distance && distance <= maxDistance) {
		panic(fmt.Errorf("match distance %d out of range", distance))
	}
	dist := uint32(distance - minDistance)
	if !(minMatchLen <= length && length <= maxMatchLen) &&
		!(dist == e.state.rep[0] && length == 1) {
		panic(fmt.Errorf(
			"match length %d out of range; dist %d rep[0] %d",
			length, dist, e.state.rep[0]))
	}
...
func (e *encoder) writeOp(op operation) error {
	...
	if op.typ == opTypeMatch {
		return e.writeMatch(op.distance, int(op.length))
	}
	return e.writeLiteral(op.lit)
}
...
var eosMatch = operation{typ: opTypeMatch, distance: maxDistance, length: minMatchLen}
...
func (e *encoder) Close() error {
	...
	if e.marker {
		if err := e.writeMatch(eosMatch.distance, int(eosMatch.length)); err != nil {
			return err
		}
	}
	err = e.re.Close()
