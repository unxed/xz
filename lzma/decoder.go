...
func (d *decoder) decodeLiteral() (s byte, err error) {
	litState := d.State.litState(d.Dict.byteAt(1), d.Dict.head)
	match := d.Dict.byteAt(int(d.State.rep[0]) + 1)
	return d.State.litCodec.Decode(d.rd, d.State.state, match, litState)
}
...
// returns the distance, the length and the literal byte of the
// operation. A literal has length zero; a match has a zero literal byte.
// The operation is not returned as operation interface value, because
// that would require a heap allocation for every decoded operation. If
// an explicit end of stream marker is identified the eos error is
// returned.
func (d *decoder) readOp() (int64, int, byte, error) {
	...
	if b == 0 {
		...
		return int64(d.State.rep[0]) + minDistance,
			int(n) + minMatchLen, 0, nil
	}
	...
	if b == 0 {
		...
		if b == 0 {
			...
			return int64(dist) + minDistance, 1, 0, nil
		}
	} else {
	...
	return int64(dist) + minDistance, int(n) + minMatchLen, 0, nil
}

// apply takes the operation returned by readOp and transforms the decoder
// dictionary accordingly.
func (d *decoder) apply(dist int64, n int, s byte) error {
	if n == 0 {
		return d.Dict.WriteByte(s)
	}
	return d.Dict.writeMatch(dist, n)
}
...
func (d *decoder) decompress() error {
	...
	for d.Dict.Available() >= maxMatchLen {
		...
		if err = d.apply(dist, n, s); err != nil {
			return err
		}
